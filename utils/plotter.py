...
class Plotter:
    ...
    def plot_metrics(self, filename='training_metrics.png'):
        ...
        epochs = range(1, len(next(iter(self.history['train'].values()))) + 1)

        # 2. Create Grid: Rows = Heads, Columns = Metric Categories (Health, Diversity, Matrix)
        fig, axes = plt.subplots(n_heads, 3, figsize=(18, 5 * n_heads))
        if n_heads == 1: axes = [axes] # Handle single head case

        for i, h_idx in enumerate(head_indices):
            suffix = f"_head_{h_idx}"
            ax_cb, ax_sub, ax_mat = axes[i]

            # --- Column 1: Codebook Health ---
