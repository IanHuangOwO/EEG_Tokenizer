...
from model.AttnRVQ.modeling_tokenizer import MultiScaleTemporalEncoder, TransformerEncoder
...
class AttnRVQPatchEmbed(nn.Module):
    """
    Converts raw EEG signal into patches using the AttnRVQ Multi-Scale approach.
    """
    ...
    def forward(self, x):
        ...
        # Sum multi-scale features (standard AttnRVQ approach)
...
class AttnRVQBackbone(nn.Module):
    def __init__(
        self,
        embed_dim=200,
        enc_depth=12,
        ...
        # 1. Patch Embedding
        self.patch_embed = AttnRVQPatchEmbed(in_chans=in_chans, embed_dim=embed_dim)
...
def attnrvq_base_patch200():
    return AttnRVQBackbone(
        embed_dim=200,
        enc_depth=12,
        enc_heads=10,
        dec_depth=4,
