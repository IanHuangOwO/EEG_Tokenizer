...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z, weight_sq_sum=None):
        # z: (Batch, Channels, D)
        z_flattened = z.view(-1, self.e_dim)
        
        # Calculate distances and indices without gradients (argmin is non-differentiable)
        with torch.no_grad():
            # If weight_sq_sum is provided, use it. Otherwise compute it.
            if weight_sq_sum is None:
                weight_sq_sum = torch.sum(self.embedding.weight**2, dim=1)
        ...
        # Quantize (Gradient flows through here)
...
class RecurrentVQ(nn.Module):
    """
    Recurrent Vector Quantizer.
    Applies the same VQ module iteratively to the residual.
    """
    def __init__(self, num_recurrent_steps, n_e, e_dim, beta=0.25):
        super().__init__()
        self.num_recurrent_steps = num_recurrent_steps
        # Single shared codebook
        self.vq = VectorQuantizer(n_e, e_dim, beta)
    ...
    def forward(self, z):
        ...
        # Precompute codebook squared norm for efficiency (reused across steps)
        # Detach or no_grad because this is only used for distance calculation (index finding)
        with torch.no_grad():
            weight_sq_sum = torch.sum(self.vq.embedding.weight**2, dim=1)

        for _ in range(self.num_recurrent_steps):
            # Reuse the same self.vq with precomputed distances
            quantized, loss, indices = self.vq(residual, weight_sq_sum=weight_sq_sum)
...
class RecurrentVQTokenizer(nn.Module):
    def __init__(
        ...
        # 3. Multi-Branch Recurrent Shared VQ
        self.rvqs = nn.ModuleList([
            RecurrentVQ(num_recurrent_steps, vocab_size, embed_dim) for _ in range(num_scales)
        ])
