...
- **Cache location:** `<backbone run folder>/feature_cache/<dataset>/<key>/<subject>.npz`, where the run folder is the parent of the checkpoint's `checkpoint/` directory (`output/pretrain/mesae_v10_small_uw01/` today). `output/` is git-ignored and the cache is regenerable, so nothing is committed from it.
