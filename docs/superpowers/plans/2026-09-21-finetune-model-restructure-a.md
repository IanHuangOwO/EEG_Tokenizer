...
**Goal:** Replace `MeSAEFeatureHead`, `MeSAEFinetune` and `PerChannelHeadAttn` with a `StampExtractor` plus three backbone-free head classes (`StampHead`, `RawBandHead`, `RawErpHead`) built from numeric head-config keys, wrapped by a small `FinetuneModel` so `train_finetune.py` keeps working unchanged apart from two lines. Checkpoints store the head config and load without shape inference.

**Architecture:** New classes go in the finetune section of `model/MeSAE/MeSAE_modules.py` (below the ADR 0016 pieces). `MeSAE.py` keeps only `FinetuneModel` (frozen backbone + extractor + head, same call signature as before: `forward(x, coords, time_idx, valid_channels, pad_mask) -> (logits, None, None)`) and `build_finetune`. `model/factory.py` gains a `channel_idx` argument and a checkpoint loader. Task 1 is additive; Task 2 switches over and deletes the old code; Task 3 sweeps docs and stale references.

**Tech Stack:** PyTorch (`eeg_fm` env), existing `viz/`, `model/factory.py`, `train_finetune.py`.

**Spec:** `docs/superpowers/specs/2026-09-21-finetune-restructure-design.md`, sub-project A (read it fully first). The spec's extractor output shape `[B, C_valid, N', S, 2]` is corrected here to **`[B, N', C_valid, S, 2]`** (patch-major, as the head math uses); Task 3 fixes the spec line. Old code for comparison: git tag `pre-head-cleanup` (`model/MeSAE/MeSAE.py` there holds `MeSAEFeatureHead`; the ADR 0016 pieces already live in `MeSAE_modules.py`).
...
`head` [`"stamp"`] one of `stamp | raw_band | raw_erp`; `granularity` [`"per_stamp"`] `per_stamp | band` (stamp head only); `spatial_k` [8]; `time_pool` [`"learned"`] `flat | learned | window`; `time_rank` [2] (`learned`); `window` [null] `[lo, hi]` seconds (`window`); `phase_advance` [false]; `evoked_rank` [0]; `dropout` [0.5].
...
- `head=stamp`, `granularity=per_stamp`: any `time_pool`; `learned` needs `time_rank >= 1` and `num_patches`; `window` needs `window`; `evoked_rank > 0` needs `num_patches` and `time_pool != "window"` (it needs the full patch axis).
- `head=stamp`, `granularity=band`: `time_pool` in `flat | window`; `phase_advance` false and `evoked_rank` 0.
- `head=raw_band`: `time_pool` in `flat | window`; `phase_advance` false; `evoked_rank` 0; `granularity` ignored.
- `head=raw_erp`: `time_pool == "flat"`; `phase_advance` false; `evoked_rank` 0; needs `num_patches`.
- Unknown keys raise; `freeze_backbone` and `backbone_lr_mult` are dropped before the check.

---

## Task 1: extractor, heads and the config resolver (additive)
...
- Produces (used by Task 2):
  ...
  - `resolve_head_config(ft_params, **derived) -> dict` (defaults, validation above, merged with `derived`)
  - `StampExtractor(backbone, channel_idx)`: `nn.Module`; registers `backbone` and buffers `keep` (alive routed + shared stamp indices) and `channel_idx`; `forward(x, coords, time_idx=None, valid_channels=None) -> amp [B, N', C_valid, S, 2]` float32; method `band_tables(sample_freq) -> (E_D, E_H)` each `[S, len(BANDS)]`.
  - `StampHead(cfg)`, `RawBandHead(cfg)`, `RawErpHead(cfg)`: `nn.Module`s with attributes `spatial` (`nn.Linear(num_channels, spatial_k, bias=False)`), `cls` (`Sequential(BatchNorm1d(n_feat), Dropout(dropout), Linear(n_feat, num_classes))`), and for `StampHead` also `time` (`LearnedTimePool` or `None`), `evoked` (`EvokedBranch` or `None`), buffers `E_D`, `E_H` (zeros `[S, 2]`, only when `granularity == "band"`). `forward(inp) -> logits`: `StampHead` takes `amp [B, N', C, S, 2]`, the raw heads take `x [B, C, N', L]` (patched raw signal, padded channels already zeroed).
  - `make_head(cfg) -> nn.Module` dispatching on `cfg["head"]`.

- [ ] **Step 1: Read first.** Read the finetune section of `MeSAE_modules.py`, `MeSAEFeatureHead` in `model/MeSAE/MeSAE.py` (`__init__` and `forward`), and `StampBank.dense_amp` / `_template_tables`. The classes below port that math exactly; if the current code differs from what this plan shows (for example the window rule or the fp32 block), the current code wins and you say so in the report.

- [ ] **Step 2: Append this code** to the finetune section of `MeSAE_modules.py` (it uses `torch`, `nn`, `overlap_add_patches`, `spatial_mix`, `FlatTimePool`, `LearnedTimePool`, `EvokedBranch`, `phase_advance` already defined in that file):

```python
BANDS = ((8.0, 13.0), (13.0, 30.0))   # mu, beta (raw_band head and the band-summed stamp head)

_HEAD_DEFAULTS = dict(head='stamp', granularity='per_stamp', spatial_k=8, time_pool='learned',
                      time_rank=2, window=None, phase_advance=False, evoked_rank=0, dropout=0.5)
...
def resolve_head_config(ft_params, **derived):
    ...
    head, tp = cfg['head'], cfg['time_pool']
    if head not in ('stamp', 'raw_band', 'raw_erp'):
        raise ValueError(f"head must be stamp|raw_band|raw_erp, got {head!r}")
    if tp not in ('flat', 'learned', 'window'):
        raise ValueError(f"time_pool must be flat|learned|window, got {tp!r}")
    if cfg['granularity'] not in ('per_stamp', 'band'):
        raise ValueError(f"granularity must be per_stamp|band, got {cfg['granularity']!r}")
    branches = cfg['phase_advance'] or cfg['evoked_rank']
    if head != 'stamp' and branches:
        raise ValueError(f"phase_advance/evoked_rank need head='stamp', got head={head!r}")
    if head == 'stamp' and cfg['granularity'] == 'band' and (branches or tp == 'learned'):
        raise ValueError("granularity='band' allows only time_pool flat|window and no phase_advance/evoked_rank")
    if head == 'raw_band' and tp == 'learned':
        raise ValueError("head='raw_band' allows time_pool flat|window only")
    if head == 'raw_erp' and tp != 'flat':
        raise ValueError("head='raw_erp' requires time_pool='flat'")
    if tp == 'window' and not cfg['window']:
        raise ValueError("time_pool='window' requires window=[lo, hi]")
    if tp == 'learned' and head == 'stamp' and int(cfg['time_rank']) < 1:
    ...
    needs_np = (head == 'raw_erp' or (head == 'stamp' and (tp == 'learned' or cfg['evoked_rank'])))
    if needs_np and cfg.get('num_patches') is None:
        raise ValueError("this head needs num_patches (trial length in patches)")
    return cfg
...
class StampExtractor(nn.Module):
    ...
    def band_tables(self, sample_freq):
        """Per-stamp template band energies (E_D, E_H), each [S, len(BANDS)], for granularity='band'."""
        with torch.no_grad():
...
def _readout(n_feat, cfg):
    # BatchNorm stands in for the probe's StandardScaler: log-powers are far from unit scale.
    return nn.Sequential(nn.BatchNorm1d(n_feat), nn.Dropout(cfg['dropout']),
...
class StampHead(nn.Module):
    """Head on the stamp code: spatial mix -> time pool -> optional branches -> readout (ADR 0014
    C0/C1/C3/C4, ADR 0016). No backbone inside."""
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        C, K, S = cfg['num_channels'], cfg['spatial_k'], cfg['num_stamps']
        N = cfg.get('num_patches')
        self.spatial = nn.Linear(C, K, bias=False)
        self.time = LearnedTimePool(int(cfg['time_rank']), S, N) if cfg['time_pool'] == 'learned' else None
        self.evoked = EvokedBranch(int(cfg['evoked_rank']), S, N) if cfg['evoked_rank'] else None
        self.flat = FlatTimePool()
        if cfg['granularity'] == 'band':
            self.register_buffer('E_D', torch.zeros(S, len(BANDS)))
            self.register_buffer('E_H', torch.zeros(S, len(BANDS)))
            n_feat = K * len(BANDS)
        else:
            n_feat = K * S * (1 + 2 * bool(cfg['phase_advance']) + 2 * bool(cfg['evoked_rank']))
        self.cls = _readout(n_feat, cfg)

    def forward(self, amp):                                                # [B, N', C, S, 2]
        cfg = self.cfg
        ...
        with torch.autocast(device_type=amp.device.type, enabled=False):
            amp = amp.float()
            if cfg['time_pool'] == 'window':
                amp = amp[:, _window_patches(cfg, amp.shape[1], amp.device)]
            ...
            if cfg['granularity'] == 'band':
                ...
                pool = self.time if self.time is not None else self.flat
                feat = torch.log(pool(a.pow(2) + b.pow(2)) + 1e-12)         # [B, K, S]
                if cfg['phase_advance']:
                    feat = torch.cat([feat, phase_advance(a, b)], dim=-1)   # ADR 0014 C3
                if self.evoked is not None:
                    feat = torch.cat([feat, self.evoked(a, b)], dim=-1)     # ADR 0014 C4
        return self.cls(feat.flatten(1))


class _RawHead(nn.Module):
    def __init__(self, cfg, n_feat):
        super().__init__()
        self.cfg = cfg
        self.spatial = nn.Linear(cfg['num_channels'], cfg['spatial_k'], bias=False)
        self.cls = _readout(n_feat, cfg)

    def _signal(self, x):                                                  # [B, C, N', L] -> [B, K, T]
        sig = overlap_add_patches(x.float(), self.cfg['patch_stride'])
        if self.cfg['time_pool'] == 'window':
            lo, hi = (int(w * self.cfg['sample_freq']) for w in self.cfg['window'])
            sig = sig[..., lo:hi]
        return spatial_mix(self.spatial, sig, 1)


class RawBandHead(_RawHead):
    """Raw-signal control: spatial mix -> mu/beta log band power -> readout (ADR 0014 raw control)."""
    def __init__(self, cfg):
        super().__init__(cfg, cfg['spatial_k'] * len(BANDS))

    def forward(self, x):
        with torch.autocast(device_type=x.device.type, enabled=False):
            s = self._signal(x)
            sp = torch.fft.rfft(s.float(), dim=-1).abs().pow(2)
            fr = torch.fft.rfftfreq(s.shape[-1], 1.0 / self.cfg['sample_freq']).to(s.device)
            feat = torch.stack([torch.log(sp[..., (fr >= lo) & (fr < hi)].sum(-1) + 1e-12) for lo, hi in BANDS], -1)
        return self.cls(feat.flatten(1))


class RawErpHead(_RawHead):
    """Raw-signal ERP baseline: spatial mix -> ~20 Hz average pooling (signed, no log) -> readout."""
    def __init__(self, cfg):
        pool = max(1, round(cfg['sample_freq'] / 20))
        T = (cfg['num_patches'] - 1) * cfg['patch_stride'] + cfg['patch_len']
        super().__init__(cfg, cfg['spatial_k'] * (T // pool))
        self.pool = pool

    def forward(self, x):
        with torch.autocast(device_type=x.device.type, enabled=False):
            feat = torch.nn.functional.avg_pool1d(self._signal(x).float(), self.pool, self.pool)
        return self.cls(feat.flatten(1))


def make_head(cfg):
    return {'stamp': StampHead, 'raw_band': RawBandHead, 'raw_erp': RawErpHead}[cfg['head']](cfg)
```

- [ ] **Step 3: Write the equivalence script** `.superpowers/sdd/2026-09-21-finetune-model-restructure-a/head_equiv.py` (uncommitted). It compares the old class (from the tag) against `StampExtractor + head`:

```python
"""Old MeSAEFeatureHead (tag pre-head-cleanup) vs new StampExtractor + head. CPU only."""
...
try:
    cfgp = load_config('output/mesae_v10_small_uw01/artifacts/config.json')
    bb = build_pretrain_from_config(cfgp)
    bb.load_state_dict(torch.load('output/mesae_v10_small_uw01/checkpoint/last.pth', map_location='cpu')['model_state_dict'])
    bb.eval()
    C, N, L, NC = 64, 39, 50, 4
    ...
    derived = dict(num_classes=NC, num_patches=N, num_channels=len(idx), patch_len=L, patch_stride=25,
                   sample_freq=200)
    # (name, old kwargs, new head keys)
    CASES = [
        ('raw mi',        dict(input='raw', task='mi', pool_channel='spatial:8', pool_time='trial'), dict(head='raw_band', time_pool='flat')),
        ('raw erp',       dict(input='raw', task='erp', pool_channel='spatial:8', pool_time='trial'), dict(head='raw_erp', time_pool='flat')),
        ('band',          dict(input='stamp_bandpow', pool_channel='spatial:8', pool_time='trial'), dict(granularity='band', time_pool='flat')),
        ('stamp flat',    dict(input='stamp_induced', pool_channel='spatial:8', pool_time='trial'), dict(time_pool='flat')),
        ('stamp learned', dict(input='stamp_induced', pool_channel='spatial:8', pool_time='learned:2'), dict(time_pool='learned', time_rank=2)),
        ('stamp window',  dict(input='stamp_induced', pool_channel='spatial:8', pool_time='window:1.0-4.0'), dict(time_pool='window', window=[1.0, 4.0])),
        ('learned+adv',   dict(input='stamp_induced', pool_channel='spatial:8', pool_time='learned:2', include_advance=True), dict(time_pool='learned', phase_advance=True)),
        ('learned+evk',   dict(input='stamp_induced', pool_channel='spatial:8', pool_time='learned:2', evoked_rank=2), dict(time_pool='learned', evoked_rank=2)),
        ('learned+both',  dict(input='stamp_induced', pool_channel='spatial:8', pool_time='learned:2', include_advance=True, evoked_rank=2), dict(time_pool='learned', phase_advance=True, evoked_rank=2)),
    ]
    def to_new_state(old_sd, head, idx):
        new = {}
    ...
    for name, okw, nkw in CASES:
        ...
        torch.manual_seed(0)
        head = M.make_head(cfg).eval()
        ext = M.StampExtractor(bb, idx).eval() if cfg['head'] == 'stamp' else None
        if ext is not None and cfg['granularity'] == 'band':
            head.E_D.copy_(ext.band_tables(200)[0]); head.E_H.copy_(ext.band_tables(200)[1])
        sd = to_new_state(old.state_dict(), head, idx)
        ...
        lo, _, _ = old(x, coords, valid_channels=vc)
        inp = ext(x, coords, valid_channels=vc) if ext is not None else x[:, idx] * vc[:, idx].float()[:, :, None, None]
        ln = head(inp)
...
```
If the old class's constructor signature or the `Old(...)` keyword arguments differ from the above (it took `input, task, pool_channel, pool_time, z_proj, dropout, sample_freq, freeze_backbone, num_patches, include_advance, evoked_rank`), adapt the call, not the comparison. `model/MeSAE/_old_mesae_tmp.py` is a temporary import target and must never be committed.
...
Expected: nine `OK <name>` lines. A mismatch is a porting bug: fix the new classes, not the test. Also assert (add to the script) that `resolve_head_config` raises `ValueError` for: `dict(head='stamp', granularity='band', time_pool='learned')`, `dict(head='raw_erp', time_pool='window', window=[1, 2])`, `dict(head='raw_band', phase_advance=True)`, `dict(foo=1)`, `dict(time_pool='window')`, and does not raise for `dict(freeze_backbone=True)`.

- [ ] **Step 5: Confirm scope and commit.** `git status --short` shows only `M model/MeSAE/MeSAE_modules.py` (the temp file is gone). Commit: `feat: StampExtractor and StampHead/RawBandHead/RawErpHead with a validated numeric head config`.
...
- Produces:
  - `FinetuneModel(backbone, head_cfg, channel_idx)` in `MeSAE.py`: attributes `backbone` (frozen), `extractor` (or `None` for raw heads), `head`, `head_cfg`, buffer `channel_idx`; `forward(x, coords, time_idx=None, valid_channels=None, pad_mask=None) -> (logits, None, None)`; `train(mode)` keeps the backbone in eval; `head_checkpoint(backbone_checkpoint) -> dict`; `FinetuneModel.from_checkpoint(backbone, ckpt) -> FinetuneModel`.
...
- [ ] **Step 1: `FinetuneModel` and `build_finetune`** in `MeSAE.py`, replacing `MeSAEFeatureHead`, `MeSAEFinetune` and the old `build_finetune` (imports needed: `StampExtractor`, `make_head`, `resolve_head_config`, `BANDS` unused; drop `PerChannelHeadAttn`):

```python
class FinetuneModel(nn.Module):
    """Frozen MeSAE backbone + one head (MeSAE_modules finetune section). Call signature matches
    the old finetune classes so train_finetune.py is unchanged: forward -> (logits, None, None)."""
    def __init__(self, backbone, head_cfg, channel_idx):
        ...
        self.extractor = StampExtractor(backbone, channel_idx) if head_cfg['head'] == 'stamp' else None
        self.head = make_head(head_cfg)
        if self.extractor is not None and head_cfg['granularity'] == 'band':
...
def build_finetune(backbone, num_channels, num_classes, channel_idx=None, num_patches=None,
    ...
    """finetune_cls entry: builds the frozen backbone + head from the numeric head config."""
    channel_idx = list(range(num_channels)) if channel_idx is None else list(channel_idx)
    num_stamps = 0
    if ft_params.get('head', 'stamp') == 'stamp':
        st = backbone.stamps
...
```
Check that `st.n_stamps - st.n_routed` is the shared-stamp count (it is what the old `torch.arange(st.n_routed, st.n_stamps)` used); `num_stamps` must equal `len(extractor.keep)` (assert this inside `FinetuneModel.__init__` for stamp heads).
...
```
(Adjust `mode` to whatever `build_finetune_from_config` passes to `build_pretrain_from_config`.)
...
- [ ] **Step 6: delete `PerChannelHeadAttn`** from `MeSAE_modules.py` (the class at the end of the pretrain part) and update the comment at about line 1001 that mentions `MeSAEFinetune.encode_post_stamp_expert`. Keep `encode_post_stamp_expert` and every other `MeSAEPretrain` method.

- [ ] **Step 7: `config/config.json` (CRLF).** In `model_params.MeSAE.finetune` replace the old keys with
  `{"head": "stamp", "granularity": "per_stamp", "spatial_k": 8, "time_pool": "learned", "time_rank": 2, "phase_advance": false, "evoked_rank": 0, "dropout": 0.5, "freeze_backbone": true}`
...
- [ ] **Step 8: verify.**
  ...
  - Extend `head_equiv.py` with a second check per case: build the new model through the real entry point, `fm = build_finetune(bb, C, NC, channel_idx=idx.tolist(), num_patches=N, sample_freq=200, dropout=0.5, **nkw)` (with `derived`-style keys already resolved inside), load the same name-mapped state with `fm.head.load_state_dict(to_new_state(old.state_dict(), fm.head, idx))`, put `fm` in `eval()`, and assert `fm(x, coords, valid_channels=vc)[0]` equals the old logits at atol 1e-6. Rerun the script: nine `OK` lines for the Task 1 check and nine for this one.
  - Checkpoint round trip (add to the script): `torch.save(fm.head_checkpoint('output/mesae_v10_small_uw01/checkpoint/last.pth'), tmp)`, then `load_finetune_checkpoint(cfgp, tmp, 'cpu')` (with `cfgp['training_params']['finetune']['pretrained_checkpoint']` set as the factory expects) reproduces the logits exactly.
...
- [ ] **Step 10: commit** in two commits: (1) `refactor: FinetuneModel with StampExtractor and head classes replaces MeSAEFeatureHead/MeSAEFinetune; checkpoints store head_config` (MeSAE.py, MeSAE_modules.py PerChannelHeadAttn removal, factory.py, viz/__init__.py, train_finetune.py, config.json); (2) `refactor: remove the dead finetune attention viz path` (base_checker.py, plugin.py).
...
**Files:**
- Modify: `CLAUDE.md`, `CONTEXT.md`, `docs/adr/0016-finetune-head-modules.md`, `docs/superpowers/specs/2026-09-21-finetune-restructure-design.md`, `docs/agents/*.md` if they name the removed classes.

- [ ] **Step 1: sweep.** Run `git grep -n -E "MeSAEFeatureHead|MeSAEFinetune|PerChannelHeadAttn|head_z|z_chan|z_proj|pool_channel|pool_time|include_advance|use_topo_feature"` and list every hit outside `docs/adr/0012*`, `docs/adr/0014*`, `docs/adr/0014_attempts.csv`, older plans under `docs/superpowers/plans/` and `config/phase2/` (historical records, left as written). Every other hit is either a stale reference to fix or a false positive; fix or list it in the report.
- [ ] **Step 2: update docs.**
  - `CLAUDE.md`: the Architecture bullets that name `MeSAEFeatureHead`, `MeSAEFinetune` and `PerChannelHeadAttn` (about lines 88 to 90) now describe `FinetuneModel`, `StampExtractor` and the three head classes; the Config section's finetune head keys become the numeric keys (list them once).
  - `CONTEXT.md` line 29: drop the `PerChannelHeadAttn` example.
  - `docs/adr/0016-finetune-head-modules.md` Consequences: the head is implemented as `StampExtractor` plus `StampHead`, `RawBandHead`, `RawErpHead` in `MeSAE_modules.py`, built from numeric keys; the checkpoint stores `head_config`; one sentence "the state-dict names listed above changed in sub-project A (no old-run compatibility)".
  - Spec: change the `StampExtractor` output shape to `[B, N', C_valid, S, 2]` in sub-project A and in the sub-project B cache description.
...
- **Spec coverage (A):** extractor, three head classes, numeric keys with validation, checkpoint with `head_config`, removals (`MeSAEFinetune`, `PerChannelHeadAttn`, `head_z`, `recon`, `z_chan`, concat, `render_finetune_attn`, dead `check_finetune` branch) are Tasks 1 and 2; real-channel-only spatial mix comes from `channel_idx`; `freeze_backbone` is dropped from the model here and from the training script in C.
- **Interfaces:** shapes are consistent: extractor `[B, N', Cv, S, 2]` feeds `StampHead`; raw heads take `[B, Cv, N', L]`; `spatial_mix(..., dim=2)` for stamp and `dim=1` for raw signals, as before.
- **Risks handled:** the equivalence script proves logits and gradients equal the old class for nine configurations under name-mapped weights (padded channels were zero, so dropping them changes nothing); `resolve_head_config` ports the old guards (`window`, `evoked_rank` and `learned` need `num_patches`, raw ERP needs flat pooling).
