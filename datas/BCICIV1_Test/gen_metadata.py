...
DATASET_INFO = {
    ...
    "description": (
        ...
        "per subject (subject-chosen pair). Evaluation (Test) runs are "
        "continuous acoustically-cued recordings with NO released cue "
        "markers -- ground truth for the eval split was never published by "
        "the competition."
    ),
    ...
    "notes": (
        ...
        "split for supervised MI finetune/eval."
    ),
}
...
def main():
    ...
    channels = load_channels(os.path.join(raw_dir, "BCICIV_eval_ds1a_1000Hz.mat"))
    structure = build_data_structure(raw_dir, r"(BCICIV_eval_ds1\w)_1000Hz\.mat")

    meta = {
        "data_metadata": {
            "dataset_name": "BCICIV1_Test",
