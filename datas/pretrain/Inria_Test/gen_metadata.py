...
DATASET_INFO = {
    ...
    "notes": (
        ...
        "(typo for PO8, preserved via original_label). Kaggle never released "
        "ground-truth ErrP labels for this split (closed competition, "
        "test.zip ships signals only) -- TestLabels.csv is a placeholder "
        "(Prediction=0 for every FeedBackEvent), lazily generated by "
        "datas/pretrain/Inria_Test/loader.py's _generate_dummy_labels() the first "
        "time it's needed. Fine for pretraining (self-supervised, ignores "
        "label values), but do NOT use this split for supervised "
        "finetune/eval of ErrP classification."
    ),
}
...
def build_data_structure(signals_dir):
    ...
    return {
        str(sub): {"signals": files, "labels": "raw/TestLabels.csv"}
