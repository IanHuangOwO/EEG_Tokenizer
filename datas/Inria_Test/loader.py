...
def _generate_dummy_labels(signals_dir: str, out_path: str) -> None:
    """
    Kaggle's Inria BCI Challenge never released ground-truth labels for the
    test split (standard for a closed competition) -- test.zip ships signals
    only, no label file. Writes a placeholder TestLabels.csv (Prediction=0 for
    every FeedBackEvent) so Loader can still cut trials for pretraining, which
    is self-supervised and does not consume label values. Do NOT treat this
    file's Prediction column as real ErrP ground truth.
    """
    import pandas as pd
    rows = []
    for fname in sorted(os.listdir(signals_dir)):
        m = re.match(r"Data_(S\d+)_(Sess\d+)\.csv", fname)
        if not m:
            continue
        subject, session = m.groups()
        df = pd.read_csv(os.path.join(signals_dir, fname))
        n_events = int((df.iloc[:, -1] == 1).sum())
        for i in range(1, n_events + 1):
            rows.append({"IdFeedBack": f"{subject}_{session}_FB{i:03d}", "Prediction": 0})

    pd.DataFrame(rows).to_csv(out_path, index=False)
    print(f"  [Inria_Test] generated placeholder {out_path}: {len(rows)} rows")
...
class Loader(BaseSubjectLoader):
    ...
    def __init__(self, config: Dict, subject_id: int, desired_channel_indices: List[int]):
        ...
        if not os.path.exists(self.label_path):
            _generate_dummy_labels(os.path.dirname(self.signal_paths[0]), self.label_path)
